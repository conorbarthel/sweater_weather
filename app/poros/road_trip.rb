class RoadTrip
  ...
  def initialize(destination_data, forecast_data)
    @id = nil
    @type = 'roadtrip'
    @start_city = "#{destination_data[:locations].first[:adminArea5]}, #{destination_data[:locations].first[:adminArea3]}"
    @end_city = "#{destination_data[:locations].last[:adminArea5]}, #{destination_data[:locations].last[:adminArea3]}"

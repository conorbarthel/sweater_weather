...
RSpec.describe 'RoadTrip create' do
  it 'happy path will create a new roadtrip', :vcr do
    ...
    expect(response[:type]).to eq('roadtrip')
    ...
    expect(response[:attributes][:travel_time]).to eq('01 hours 22 min')
    expect(response[:attributes][:weather_at_eta]).to be_a(Hash)
    expect(response[:attributes][:weather_at_eta][:temperature]).to eq(61.8)
    expect(response[:attributes][:weather_at_eta][:conditions]).to eq('broken clouds')
  ...
  it "sad path will not create a roadtrip if roadtrip is impossible", :vcr do
    user = User.create!(email:"somethingelse@mail.com", password: 'password', password_confirmation: 'password')
    bad_body = {
                      origin: "Denver,CO",
    ...
    post '/api/v1/roadtrips', params: bad_body
    parsed = JSON.parse(response.body, symbolize_names: true)
